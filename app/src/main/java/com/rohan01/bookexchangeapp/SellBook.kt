...
class SellBook: AppCompatActivity() {
    ...
    private fun store()
    {
       val nameofbook = findViewById<View>(R.id.editTextBookName) as EditText
        ...
        val linkofbook = findViewById<View>(R.id.editTextUploadLink)as EditText
        ...
        val booklink = linkofbook.text.toString().trim()

        if(!bookname.isEmpty() && !bookpoints.isEmpty() && !booklink.isEmpty() && !sellername.isEmpty() ){
           try {
               val items = HashMap<String, Any>()
               items.put("BookName",bookname)
               items.put("Points",bookpoints)
               items.put("SellerName",sellername)
               items.put("BookLink",booklink)
               db.collection("SellOffer").document().set(items).addOnSuccessListener {
                  void:  Void? -> Toast.makeText(this,"Successfully updated ",Toast.LENGTH_SHORT).show()
               }.addOnFailureListener { exception: java.lang.Exception -> Toast.makeText(this, exception.toString(), Toast.LENGTH_SHORT).show()  }
           }catch (e: Exception) {
